...
def _normalize_snapshot_path(path: str | None) -> str | None:
    """Return a sanitized value suitable for storing in metadata.

    The SQL query used by :func:`test_no_tmp_snapshot_paths_ci_only` treats any
    ``snapshot_path`` beginning with ``/tmp/`` as a failure.  Storing absolute
    paths under the system temporary directory is noisy in local development and
    confusing in CI (where ``tmp`` is often the default runner area).  We
    therefore normalize any such path to its basename before writing it into
    the database.  This keeps the metadata useful (the file name is still
    recorded) while avoiding accidental ``/tmp`` references that trigger the
    CI guard.

    Parameters
    ----------
    path: Optional[str]
        Original snapshot path as generated by the pipeline or other callers.

    Returns
    -------
    Optional[str]
        Sanitized path string, or ``None`` if the input was ``None``.
    """
    if path is None:
        return None
    ...
    try:
        ...
        # if the path is somehow unparsable, just return the raw value so
        # upstream code can surface the problem instead of masking it.
        return path
    ...
    if "snapshot_path" in metadata:
        ...
        if isinstance(raw_path, str):
            try:
                ...
                if raw_path.startswith(prefix) or raw_path == str(tmpdir):
                    metadata["snapshot_path"] = Path(raw_path).name
                else:
                    metadata["snapshot_path"] = _normalize_snapshot_path(raw_path)
            except Exception:
