# Implantação — Story 0.3: Pre-commit (ruff)
...
- `.pre-commit-config.yaml` com hooks para `ruff` e alguns hooks básicos (`end-of-file-fixer`, `trailing-whitespace`).
- Atualização de `pyproject.toml` com configurações de `tool.ruff`.
- Workflow GitHub Actions: `.github/workflows/ci.yml` que executa `pre-commit` e `ruff check`.
...
- pyproject.toml (atualizado: `tool.ruff`)
...
- Mantivemos `line-length = 88` para compatibilidade com a configuração de lint/format atual e evitar churn em formatação.
...
- `pre-commit`: hooks principais (`ruff`, `pre-commit-hooks`) passaram após aplicação de formatações automáticas.
...
- Uso de `pre-commit` com hooks para `ruff`:
...
- Pinagem de `rev` em `.pre-commit-config.yaml`:
  - Objetivo: garantir reprodutibilidade dos hooks (`ruff` v0.14.14).
...
- Alinhamento de versões em `pyproject.toml`:
  - Objetivo: manter as dependências de desenvolvimento (`ruff`) consistentes entre ambiente local e CI.
...
- Manter `line-length = 88`:
  - Objetivo: compatibilidade com a configuração de lint/format atual e evitar churn em formatação.
  - Por quê: `88` é usado como limite consistente no projeto; mudar exige consenso e atualização ampla no repositório.
