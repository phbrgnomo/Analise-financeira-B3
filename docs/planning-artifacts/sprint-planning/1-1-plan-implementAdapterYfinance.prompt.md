...
TL;DR — Criar interface `Adapter` e um adaptador `yfinance` que retorne um `pandas.DataFrame` com metadados `source` e `fetched_at`. Usaremos `yfinance` como provider padrão (não depender de `pandas_datareader` devido à incompatibilidade com Python 3.12). Testes unitários usam fixtures CSV existentes para evitar chamadas de rede.
...
3. Implementar adaptador provider:
   ...
   - Caso `yfinance` não esteja instalado, falhar com erro claro informando para instalar `yfinance` (não usar `pandas_datareader`, que não é suportado em Python 3.12).
...
5. Testes:
   ...
   - Testes:
   - Mockar `yfinance.download` (ou função interna) para retornar DataFrame carregado de `tests/fixtures/sample_ticker.csv`.
...
Decisions
 - Dependência: usar `yfinance` (recomendado). Não depender de `pandas_datareader` por problemas de compatibilidade com Python 3.12.
